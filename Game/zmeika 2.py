...
display = pygame.display.set_mode((display_width, display_height + 30))  # Увеличение высоты для счета и рекорда
pygame.display.set_caption("Родимая змейка")  # Название окна

clock = pygame.time.Clock()  # Функция для управления скоростью игры

zmeika_block = 15  # Размер змейки
zmeika_speed = 15  # Скорость змейки

text_end = pygame.font.SysFont("TkTooltipFont", 21)  # Размер шрифта после поражения
text_point = pygame.font.SysFont("comicsansms", 21)  # Размер шрифта счета очков
...
def our_point_or_record(point, record):  # Функция для отображения очков и рекорда
    score_text = text_point.render("Point: " + str(point) + "   Record: " + str(record), True, color_point)
...
def gameOsnova():  # Основная функция по игре
    ...
    apple_x = round(random.randrange(0, display_width - zmeika_block) / 15.0) * 15.0  # Рандомное появления яблока x
    apple_y = round(random.randrange(0, display_height - zmeika_block) / 15.0) * 15.0  # Рандомное появления яблока y
    ...
    while not game_over:
        ...
        display.blit(apple3_img, (apple_x, apple_y))  # Добавления яблока на экран
        pygame.draw.rect(display, color_apple, [apple_x, apple_y, zmeika_block, zmeika_block])#рисует яблоки 
        display.blit(apple3_img, (apple_x, apple_y))  # Добавления яблока на экран
        ...
        our_zmeika(zmeika_block, zmeika_List)#вызывают функцию для ьотображения змейки 
        ...
        if x1 == apple_x and y == apple_y:#цикл по поеданию яблок 
            apple_x = round(random.randrange(0, display_width - zmeika_block) / 15.0) * 15.0
            apple_y = round(random.randrange(0, display_height - zmeika_block) / 15.0) * 15.0
            Length_of_zmeika += 1
